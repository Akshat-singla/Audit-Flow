...
export default function Home() {
    ...
    const [rightPanelTab, setRightPanelTab] = useState<'workflow' | 'interact' | 'events'>('workflow');
    ...
    useEffect(() => {
        ...
        // Show security report after analysis step completes (BEFORE compilation)
        if (currentStep?.id === 'analyze' && currentStep.status === 'completed' && workflow.analysisResult) {
    ...
    const handleSecurityContinue = useCallback(async () => {
        setShowSecurityReport(false);
        // Continue to compilation step
        try {
            await workflowController.continueAfterAnalysis();
        } catch (error) {
            console.error('Failed to continue after analysis:', error);
        }
    }, []);

    const handleSecurityFixIssues = useCallback(() => {
        setShowSecurityReport(false);
        // Reset workflow to allow user to edit the contract
        workflowController.resetWorkflow();
        toastManager.info('Workflow reset. You can now edit your contract and restart.');
    }, []);
                        ...
                        <div className="border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
                            <div className="flex">
                                <button
                                    ...
                                    className={`flex-1 px-4 py-3 text-sm font-medium transition-colors ${rightPanelTab === 'workflow'
                                ...
                                <button
                                    ...
                                    className={`flex-1 px-4 py-3 text-sm font-medium transition-colors ${rightPanelTab === 'interact'
                                        ? 'text-blue-600 dark:text-blue-400 border-b-2 border-blue-600 dark:border-blue-400'
                                        : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100'
                                        }`}
                                >
                                ...
                                <button
                                    ...
                                    className={`flex-1 px-4 py-3 text-sm font-medium transition-colors ${rightPanelTab === 'events'
                                        ? 'text-blue-600 dark:text-blue-400 border-b-2 border-blue-600 dark:border-blue-400'
                                        : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100'
                                        }`}
                                >
                        ...
                        <div className="flex-1 overflow-hidden">
                            ...
                            {rightPanelTab === 'interact' && (
                                <div className="h-full overflow-y-auto p-4">
                                    {workflow?.deploymentResult && workflow?.compilationResult?.abi && selectedNetworkId ? (
                                        <ContractInteraction
                                        ...
                                        <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                                            <p>Deploy a contract to interact with it</p>
                                        </div>
                                    )}
                                </div>
                            )}
                            ...
                            {rightPanelTab === 'events' && (
                                <div className="h-full overflow-y-auto p-4">
                                    {workflow?.deploymentResult && workflow?.compilationResult?.abi && selectedNetworkId ? (
                                        ...
                                        <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                                            <p>Deploy a contract to monitor events</p>
                                        </div>
                                    )}
                                </div>
                            )}
